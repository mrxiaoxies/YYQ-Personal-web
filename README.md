...
`0.3.0` 将个人经历助手从“命中单条相似资料”升级为“主题聚合 + 事实推导”：

- `knowledge/index.json` 增加五类主题与结构化工作月份；宽泛问题按主题自动汇总多条公开证据，具体项目和具体能力仍使用原有细粒度条目。
- 新增 Netlify `/api/ask` 与 `/api/health` 接口，通过 AI Gateway 生成基于证据的回答，并由服务端返回资料来源。
- 使用本地 `bge-small-zh-v1.5` 同时生成 16 条知识向量和 5 个主题向量；详细问法沿用关键词/语义 RRF，宽泛主题按语义、词法、子知识支持和证据覆盖做固定加法评分。
- 新增可追溯“事实推导”，从公开结构化资料计算从业跨度、计数、分组、关联和阶段比较；计算值由服务端加入回答，模型不能自行改写数字。
- 回答模型的每条结论必须返回公开来源 ID，服务端验证后才展示；未知工具、未知框架和跨项目技术拼接仍直接拒答。
- “查看检索过程”新增主题、聚合证据数和事实推导类型；不会公开原始向量、事实值、来源 ID、知识正文、提示词或错误详情。
